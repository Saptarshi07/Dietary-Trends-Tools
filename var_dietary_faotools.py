...
#returns the data of consumption by area_code at a year between 1961 to 2013
def val_groups_consumed(area_code,year):
    vals = {'fruits':  0,
     'vegetables': 0,
     'grains': 0,
     'meats': 0,
    ...
    for i in food_groups:
        for j in food_groups[i]:
            vals[i] += get_estimated_data(fbn,area_code,j,664,year)
    return vals
    


#land-used if area_code at a certain year had a certain diet that is given by calorie_val_dist
...
#land-used due to consumption by actual consumption of area_code at that country
def get_total_land_used(area_code,year):
    val = {'local':0, 'remote':0, 'total':0}
    for i in food_groups:
        x = get_land_saved_var(area_code,i,0,year)
        val['local'] += x['local']
        val['total'] += x['total']
        val['remote'] += x['remote']
    return val  
...
def get_masses_per_group_var(area_code, group, group_cal, year):


#in tonnes
    
    pop = get_estimated_data(foodbalancepd,area_code,2501,511,year)*1000
    calorie_breakdown = group_cal
    dictionary = {}
    calorie_parts = {}
    calorie_distribution = {}
    mass_per_sub_group = {}
    for sub_group in food_groups[group]: 
        val = get_estimated_data_for(fbn,area_code,sub_group,664,year)
    ...
    for sub_group in food_groups[group]:
        ...
        val_cal = get_estimated_data_for(fbn,area_code,sub_group,664,year)
...
def get_land_saved_var(area_code, group, group_cal, year):
    ...
    for sub_group in food_groups[group]:
        ...
        rec_import = idr*get_masses_per_group_var(area_code, group, group_cal, year)[sub_group]
        rec_food =  get_masses_per_group_var(area_code, group, group_cal, year)[sub_group] - rec_import
...
def get_pastoral_mixed_landless(area_code,item_code,year):
    ...
    for k in stocks.keys():
        ...
        _prod = get_estimated_data(livestockpd,area_code,k,production_code,year)
        _carcass = get_estimated_data_for(livestockpd,area_code,k,carcass_weight_code,year)*0.0001
        _offtake = get_estimated_data(livestockpd,area_code,k,producing_animals_code,year)
...
def get_cropland_feed_area(area_code,item_code,year):
    ...
    for feed_element in feed_items_in_crops:
        ...
        _yield = get_estimated_data_for(cropspd,area_code,feed_element,yield_code,year)
